...
class CareBeneficiaryChatController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('carebeneficiary.pages.chat-index', compact('chats'));
    }
    ...
    public function show($chatId)
    {
        ...
        if (!$chat->users->contains(Auth::id())) {
            return redirect()->route('carebeneficiary.chat.index')->with('error', 'You are not part of this chat.');
        }

        return view('carebeneficiary.pages.chat-show', compact('chat'));
    }
    ...
    public function CreateChat()
    {
        ...
        return view('carebeneficiary.pages.chat-create', compact('users'));
    }
    ...
    public function storeChat(Request $request)
    {
        ...
        if (count($userIds) == 2) {
            ...
            if ($existingChat) {
                return redirect()->route('carebeneficiary.chat.show', $existingChat->id)->with('success', 'Existing chat found.');
            }
        }
        ...
        return redirect()->route('carebeneficiary.chat.show', $chat->id)->with('success', 'Chat created successfully.');
    }
