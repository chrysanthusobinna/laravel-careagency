                    ...
                    <div class="card-body pt-0 recent-orders px-0">
                        <div class="table-responsive theme-scrollbar">
                            ...
                            <div class="alert txt-primary border-warning alert-dismissible fade show m-3 p-3 d-flex align-items-center" role="alert">
                                ...
                                <span class="text-danger">No Family Members found</span>
                            </div>
                            @else
                            <div class="table-responsive theme-scrollbar p-3">
                                ...
                                <table class="table display mt-3" id="recent-orders" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        @php
                                            $randomColor = isset($colorClasses) && is_array($colorClasses) ? $colorClasses[array_rand($colorClasses)] : 'primary';
                                            $beneficiary = $family->careBeneficiary;  
                                        @endphp
                                        <tr>
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    <div class="flex-shrink-0">
                                                        @if(isset($family->careBeneficiary->profile_picture) && $family->careBeneficiary->profile_picture == 'default.png')
                                                            <div class="letter-avatar">
                                                                <h6 class="txt-{{ $randomColor }} bg-light-{{ $randomColor }}">{{ optional($family->careBeneficiary)->initials }}</h6>
                                                            </div>
                                                        @else
                                                            <img src="{{ asset('uploads/profile_pictures/' . optional($family->careBeneficiary)->profile_picture) }}" alt="Profile Picture" class="img-fluid rounded-circle" width="40">
                                                        @endif
                                                    </div>
                                                    <div class="flex-grow-1">
                                                        <a href="{{ route('serviceuser.family-member.show', optional($family->careBeneficiary)->id) }}">
                                                            <h6>{{ optional($family->careBeneficiary)->first_name . " " . optional($family->careBeneficiary)->last_name }}</h6>
                                                        </a>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    <div class="flex-shrink-0">
                                                        @if(isset($family->familyMember->profile_picture) && $family->familyMember->profile_picture == 'default.png')
                                                            <div class="letter-avatar">
                                                                <h6 class="txt-{{ $randomColor }} bg-light-{{ $randomColor }}">{{ optional($family->familyMember)->initials }}</h6>
                                                            </div>
                                                        @else
                                                            <img src="{{ asset('uploads/profile_pictures/' . optional($family->familyMember)->profile_picture) }}" alt="Profile Picture" class="img-fluid rounded-circle" width="40">
                                                        @endif
                                                    </div>
                                                    <div class="flex-grow-1">
                                                        <a href="{{ route('serviceuser.family-member.show', optional($family->familyMember)->id) }}">
                                                            <h6>{{ optional($family->familyMember)->first_name . " " . optional($family->familyMember)->last_name }}</h6>
                                                        </a>
                                                    </div>
                                                </div>
                                            </td>
            ...
            <form method="POST" action="{{ route('admin.family-members.update') }}">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="relationship_type" id="update_relationship">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
